...
class MultiheadAttention(nn.Module):
    def __init__(self, d_model, n_heads, d_k=None, d_v=None, res_attention=False, 
                 attn_dropout=0., proj_dropout=0., qkv_bias=True, lsa=False, use_lora=False):
        ...
        if use_lora:
            self.W_Q = lora.Linear(d_model, d_k * n_heads, bias=qkv_bias, r=16)
            self.W_K = lora.Linear(d_model, d_k * n_heads, bias=qkv_bias,r=16)
            self.W_V = lora.Linear(d_model, d_v * n_heads, bias=qkv_bias, r=16)
        else:
        ...
        # Poject output
        if use_lora:
            self.to_out = nn.Sequential(lora.Linear(n_heads * d_v, d_model), nn.Dropout(proj_dropout))
        else:
...
class TSTEncoderLayer(nn.Module):
    def __init__(self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=128, store_attn=False,
                 ...
                 res_attention=False, pre_norm=False, use_lora=False):
        super().__init__()
        ...
        self.self_attn = MultiheadAttention(d_model, n_heads, d_k, d_v, 
                                            ...
                                            res_attention=res_attention, use_lora=use_lora)
...
class TSTEncoder(nn.Module):
    def __init__(self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=None,
                        ...
                        res_attention=False, n_layers=1, pre_norm=False, store_attn=False, use_lora=False):
        super().__init__()

        self.layers = nn.ModuleList([TSTEncoderLayer(q_len, d_model, n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
                                                      attn_dropout=attn_dropout, dropout=dropout,
                                                      activation=activation, res_attention=res_attention,
                                                      pre_norm=pre_norm, store_attn=store_attn, use_lora=use_lora) for i in range(n_layers)])
        self.res_attention = res_attention
...
class TSTiEncoder(nn.Module):  #i means channel-independent
    def __init__(self, c_in, patch_num, patch_len, max_seq_len=1024,
                 ...
                 pe='zeros', learn_pe=True, verbose=False, use_lora=False, **kwargs):
                 #act="gelu"
        ...
        # Positional encoding
        self.W_pos = self.__positional_encoding(pe, learn_pe, q_len, d_model)
    ...
    def __coord1d_pos_encoding(self, q_len, exponential=False, normalize=True):
    ...
    def __positional_encoding(self, pe, learn_pe, q_len, d_model):
        # Positional encoding
        if pe == None:
        ...
        elif pe == 'lin1d': W_pos = self.__coord1d_pos_encoding(q_len, exponential=False, normalize=True)
        elif pe == 'exp1d': W_pos = self.__coord1d_pos_encoding(q_len, exponential=True, normalize=True)

        else: raise ValueError(f"{pe} is not a valid pe (positional encoder. Available types: 'gauss'=='normal', \
            'zeros', 'zero', uniform', 'lin1d', 'exp1d', None.)")
...
class PatchTST(nn.Module):
    def __init__(self, c_in, context_window, target_window, patch_len, stride, max_seq_len:Optional[int]=1024,
                 ...
                 verbose:bool=False, use_lora:bool=False, **kwargs):
        ...
        self.backbone = TSTiEncoder(c_in, patch_num=patch_num, patch_len=patch_len, max_seq_len=max_seq_len,
                                ...
                                pe=pe, learn_pe=learn_pe, verbose=verbose, use_lora=use_lora,**kwargs)
